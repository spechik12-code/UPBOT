...
print(f"Загружено {len(accounts)} аккаунтов. Playwright бот — стабильный.")
    ...
    with sync_playwright() as p:
        ...
        try:
            for idx, acc in enumerate(accounts):
                ...
                # Защита 18+ — кликаем даже скрытую
                if page.locator("//button | //div[contains(@style, 'cursor: pointer')]").count() > 0:
                    print("Защита найдена — кликаем (force=True)")
                    page.locator("//button | //div[contains(@style, 'cursor: pointer')]").first.click(force=True)
                    time.sleep(5)

                # После защиты — на логин
                if "online-escorts" in page.url:
                    print("После защиты — переходим на логин вручную")
                    page.goto("https://43xgeorgia.me/wp-login.php", wait_until="networkidle")
                    time.sleep(5)
                ...
                if up_link.count() > 0:
                    ...
                    page.goto(up_url, wait_until="networkidle")
                    print(f"[{datetime.now(TBILISI_TZ).strftime('%H:%M')}] UP УСПЕШНО: {acc['login']} 🎉")
                    time.sleep(5)

                # Защита после UP
                if page.locator("//button | //div[contains(@style, 'cursor: pointer')]").count() > 0:
                    print("Защита после UP — кликаем (force=True)")
                    page.locator("//button | //div[contains(@style, 'cursor: pointer')]").first.click(force=True)
                    time.sleep(5)
                    # Повторный UP
                    up_link = page.locator("a.k-up.send").first
                    if up_link.count() > 0:
                        page.goto(up_link.get_attribute("href"), wait_until="networkidle")
                        print(f"Повторный UP после защиты: {acc['login']} 🎉")
                ...
                if logout_link.count() > 0:
                    page.goto(logout_link.get_attribute("href"), wait_until="networkidle")
                    print("Логаут выполнен")
                    time.sleep(4)
                else:
                    print("LogOut не найден — прямой URL")
                    page.goto("https://43xgeorgia.me/wp-login.php?action=logout", wait_until="networkidle")
                    time.sleep(5)
                    try:
                        page.click("//a[contains(@href, 'action=logout') and contains(text(), 'log out')]", force=True)
                    except:
                        pass
